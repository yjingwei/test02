...
class word2vec():
    def __init__(self,
                 ...
                 num_sampled=1000,
                 learning_rate=1.0,
                 logdir='./simple_word2vec',# tensorboard记录地址
                 model_path=None
                 ):
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            # 将输入序列向量化
            embed = tf.nn.embedding_lookup(self.embedding_dict, self.train_inputs)  # batch_size,从W_1矩阵中直接进行选择
            ...
            self.normed_embedding = self.embedding_dict / vec_l2_model#shape(3000,100)
            # self.embedding_dict = norm_vec # 对embedding向量正则化
            test_embed = tf.nn.embedding_lookup(self.normed_embedding, self.test_word_id)
            self.similarity = tf.matmul(test_embed, self.normed_embedding, transpose_b=True)#shape(?,3000)
